...
begin
	...
	if (LEN(@sdt)<10 or len(@sdt)>12)
		...
		End
End;
INSERT INTO KHACHHANG VALUES('KH09', N'Ngô Hoài Nhật ', '05-12-2002', 'Nam', 15963259, 03124567897, N'Việt Nam', N'123 Lê Lợi, HCM')
...
begin
	...
	begin
		if @ngaylapHD <= GETDATE()
			insert into HOADON values (@idHD,@idKhach,@idPhieudat,@idNhanvien,@ngaylapHD,@trangthaiHD,@tongtien)
		else
			print N'Ngày lập hoá đơn phải nhỏ hơn ngày hiện tại'
		end
end

exec themHoadon 'HD01','KH01','PH01','@NV01','10-12-2022',N'Chưa thanh Toán',3000000

create trigger sdtNV on [dbo].[NHANVIEN] for insert, update
as
begin
	declare @sdtNV nvarchar(12)
	select @sdtNV = (select SDT from inserted)
	if (LEN(@sdtNV)<10 or len(@sdtNV)>12)
		begin
			print N'Số điện thoại không phù hợp'
			Rollback tran
		End
...
create trigger CMNDKhach on [dbo].[KHACHHANG] for insert, update
as
begin
	declare @cmndKH nvarchar(12)
	select @cmndKH = (select CMND from inserted)
	if (LEN(@cmndKH)<10 or len(@cmndKH)>12)
		begin
			print N'Số chứng minh nhân dân không phù hợp'
			Rollback tran
		End
End;

create trigger CMNDNV on [dbo].[NHANVIEN] for insert, update
as
begin
	declare @cmndNV nvarchar(12)
	select @cmndNV = (select CMND from inserted)
	if (LEN(@cmndNV)<10 or len(@cmndNV)>12)
		begin
			print N'Số chứng minh nhân dân không phù hợp'
			Rollback tran
		End
...
INSERT INTO KHACHHANG VALUES('KH10', N'Ngô Hoài Nhật ', '05-12-2002', 'Nam', 159632592012, 03124567897, N'Việt Nam', N'123 Lê Lợi, HCM')
INSERT INTO NHANVIEN VALUES('NV05', N'HÀ', '05-12-2002', 123456123456, 'TP.HCM', 'hao@email', '0123456789', 'hao', '123')
